...
class FaceScanViewController: UIViewController, ARSessionDelegate {
    ...
    private func export(node: SCNNode) {
        ...
        do {
            ...
            // Fallback: try using SCNSceneSource
            DispatchQueue.main.async {
    ...
    private func exportFallback(scene: SCNScene, url: URL) {
        // Alternative export method using USDZ export
        do {
            ...
            // For production, you might want to use RealityKit's export or a third-party converter
            // For now, we'll try to use the written file
            ...
            print("Fallback export also failed: \(error)")
        }
